                ...
                for p in range(1,l-1+1):
                    ...
                    for a in t1:
                        for b in t2:
                            ...
                            if r is not None:
                                for w in r:
                                    #cprint('Applied Rule: ' + str(w) + '[' + str(l) + ',' + str(s) + ']' + ' --> ' + str(a.get_type) + '[' + str(p) + ',' + str(s) + ']' + ' ' + str(b.get_type)+ '[' + str(l-p) + ',' + str(s+p) + ']')
...
g = Grammar(Path('grammar/GramaticaTokiPona2CRF.txt'))

# Parse a sentence
texto = input()
frases =texto.replace('?','.').replace('!','.').replace(':','.').replace('"','').replace(',','').split('.')
